...
with open('Z2.txt', 'wb') as f:
    embeddings = inputVectors + outputVectors
    for a in range(V):
      f.write(str(embeddings[a]))
print ("embeddings written to file")
# print inputVectors
print 'outputVector5'
print outputVectors[5]
zs=str(outputVectors)
fz=file('Z.txt','wb')
fz.write(zs)
fz.close()
import pprint
...
features_matrix = np.asarray([model[str(node)] for node in range(100)])
